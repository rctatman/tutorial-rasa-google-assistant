...
class ActionPlaceSearch(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        get_origin = requests.post(
            "https://www.googleapis.com/geolocation/v1/geolocate?key={}".format(key)
        ).json()
        print(get_origin)
        ...
        place = requests.get(
            "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={},{}&radius={}&type={}&key={}".format(
                origin_lat, origin_lng, radius, query, key
            )
        ).json()
        if len(place["results"]) == 0:
            ...
            for i in place["results"]:
                if "rating" and "vicinity" in i.keys():
                    ...
                    break
        speech = "I found a {} called {} based on your specified parameters.".format(
            query, name
        )
        dispatcher.utter_message(speech)  # send the response back to the user
        return [
            SlotSet("location_match", "one"),
            SlotSet("rating", rating),
            SlotSet("address", address),
            SlotSet("opening_hours", opening_hours),
